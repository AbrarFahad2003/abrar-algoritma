...
public:
    Stack() : top(NULL) {}  // Menggunakan NULL sebagai pengganti nullptr

    // Menambahkan data mahasiswa ke stack
    void push(const Mahasiswa) {
        Mahasiswa* newNode = new Mahasiswa;
        newNode->next = top;
        top = newNode;
        cout << "Data mahasiswa " <<nama << " berhasil ditambahkan.\n"<<endl;
    }
    ...
    void sisipTerurut(const Mahasiswa) {
        Mahasiswa* newNode = new Mahasiswa;
        if (top == NULL || top->npm > mhs.npm) {
            newNode->next = top;
            top = newNode;
            return;
        }
